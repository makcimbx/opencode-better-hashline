...
- `PARTIAL_PUBLICATION` может оставить созданные этим вызовом каталоги, новый файл или обе стороны частичного перемещения; небезопасный автоматический откат намеренно не выполняется, а уже ожидающие операции с пересекающимися путями останавливаются до изменения.
