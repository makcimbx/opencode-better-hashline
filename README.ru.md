...
Префиксы являются аннотациями, а не содержимым файла. `offset` начинается с единицы, `limit` по умолчанию равен 1 000 и принимает значения до 100 000. Фактический объём ответа ограничивает `maxOutputBytes`: неполная страница помечается `partial=true` и заканчивается `@more`.

Строка вида `N!|... [preview only; line not issued]` слишком велика для выданной страницы и не может быть изменена по ссылке. Несколько подтверждённых страниц одного неизменившегося снимка могут вместе накопить полное покрытие файла.
...
`hashline_write` предназначен только для создания новых файлов. `createParents:true` явно разрешает создать до 64 отсутствующих каталогов по заранее зафиксированному плану. После частичной публикации автоматический откат не выполняется: возвращается `PARTIAL_PUBLICATION`, а дерево нужно проверить перед повтором.
