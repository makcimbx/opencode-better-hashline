...
Для единственной операции `replace_file` отсутствие `finalNewline` сохраняет состояние снимка, если `lines` не пуст. Само `lines: []` создаёт пустой файл, автоматически подразумевая `finalNewline:false`; явное `true` с пустым массивом остаётся недопустимым.

`readback:true` запрашивает одну страницу результата с окном по умолчанию. Наличие `readbackOffset` или `readbackLimit` само включает readback, поэтому отдельный флаг не нужен; явное `readback:false` вместе с окном недопустимо.
...
Lifecycle-операции не возвращают readback и отклоняют `readback:true`, `readbackOffset` и `readbackLimit`. `move_file` по-прежнему требует существующего родителя, не создаёт каталоги и не перезаписывает назначение.
...
Последняя неизменяемая model-free запись: [schema-v8 default simplification](benchmarks/results/2026-07-23-default-simplification-r2-windows-x64.json). Корпус из 29 сценариев покрывает точные и устаревшие изменения, коллизии, неоднозначность, границы, конфликты, кодировки, переносы и составное перемещение; результаты schema-v5, schema-v6 и schema-v7 остаются неизменяемой историей.
