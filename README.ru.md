...
Для единственной операции `replace_file` отсутствие `rebase` означает строгий `none`. Отсутствие `finalNewline` сохраняет состояние снимка, если `lines` не пуст. Само `lines: []` создаёт пустой файл, автоматически подразумевая `finalNewline:false`; явное `true` с пустым массивом остаётся недопустимым.
...
Для lifecycle-операций отсутствие или явное значение `rebase: "none"` означает строгую побайтовую проверку; `unique` запрещён. Они не возвращают readback и отклоняют `readback:true`, `readbackOffset` и `readbackLimit`. `move_file` по-прежнему требует существующего родителя, не создаёт каталоги и не перезаписывает назначение.
...
Последняя неизменяемая model-free запись: [schema-v9 operation-aware rebase](benchmarks/results/2026-07-23-operation-aware-rebase-default-windows-x64.json). Корпус из 29 сценариев покрывает точные и устаревшие изменения, коллизии, неоднозначность, границы, конфликты, кодировки, переносы и составное перемещение. Новый omitted/default adapter использует incremental-ветвь того же общего resolver политики, что и плагин; strict-only defaults проверяются runtime-тестами, а не этим корпусом. Исторический результат эпохи 0.7.0 schema-v8 и все предыдущие записи остаются неизменяемыми.
