...
- The native-alias pilot uses the frozen `--native-alias-pilot` manifest. Pilot v2 is retired unexecuted; pilot v3 consumed its reservation and failed closed after one session; pilot v4 consumed its reservation and failed closed after two sessions. Neither may resume or retry. Any v5 candidate must start from a new null anchor and produce exact receipt/artifact/package-tree/staged-runner evidence; external bundle B must bind auth, endpoint, hard-budget, user approval, toolchain, schedule, and broker hashes; direct-child commit C may change only the anchor to B's hash and must reuse A's runner bytes. The approved external broker must atomically reserve v5 outside every repository/worktree before any model process. Never treat development probes, deterministic checks, or packed evidence as the paid release gate.
