...
- The retained schema-v9 model-free result measures explicit strict, explicit unique, and the incremental branch of operation-aware omitted rebase on the unchanged 29-case textual corpus; strict-only defaults are runtime-test evidence, not corpus evidence. Keep schema-v5 through schema-v9 and pilot-v7 evidence immutable; make no semantic-code, paid, or model-quality claim from deterministic output.
