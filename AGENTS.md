...
- Every `@hashline` header reports `coverage=partial|complete`, computed from evidence issued when it is rendered plus the candidate page. `complete` means those inputs are sufficient; that completeness is monotonic if the candidate stays valid and is delivered and attested. `partial` means they were insufficient at render time, but another page delivered and attested later or out of order may complete the snapshot and make that prediction conservative. `partial=true` remains page-local, so `partial=true coverage=complete` is valid. Rendering and pending output never issue refs, invalidated candidates issue nothing, and headers contain no coverage ranges.
- Exact retained bytes are freshness authority. Omitted `rebase` resolves to exact, ambiguity-rejecting `"unique"` for batches limited to `replace`, `insert`, `copy_range`, and `move_range`, and to strict `"none"` for sole `replace_file`, `delete_file`, and `move_file`. Explicit `"none"` requires full-byte freshness; explicit `"unique"` retains the same incremental-only constraints. Unique proves textual identity, not semantic independence or edit-history causality. Do not add fuzzy matching, normalization, nearest-match selection, source repair, or silent fallback.
...
- Text readback is one optional contiguous successor page. For text edits, including sole strict `replace_file`, `readback:true` or either window field explicitly requests it; omission requests none, and a window with explicit `readback:false` is invalid. Only `delete_file` and `move_file` are lifecycle operations; they reject `readback:true` and all windows and never attach successors. Only an after-hook-attested delivered page issues refs, and there is no ID-only successor.
- Keep `hashline` as the default unique-ID surface and native `read`. Experimental `native-aliases` requires `enforce:true`, compatible host capabilities, exact observed package/schema/fingerprint/worktree identity, a process-local live epoch established only by a delivered and attested `hashline_read`, and no silent fallback; it never aliases native `write`, and both source and destination mutations must remain inside the current worktree. Neither surface is a shell sandbox.
- Keep the native-alias marker name `native-aliases/v2`, but treat every package/schema/fingerprint identity change as live-epoch-incompatible. Restart as required and use a fresh delivered `hashline_read` to rebind in the same session; old snapshot IDs remain unusable.
...
- Preserve the filesystem order: canonicalize and authorize, plan one immutable result, approve that exact diff, reread bytes/identity, then stage and publish. Exact same-envelope identity refresh and write-parent suffix contraction are allowed only under the original locks and before edit approval; never replan after approval.
- File lifecycle operations stay sole, strict, complete-coverage mutations outside `planEdits`. Under the original canonical locks they may refresh exact same-path source/destination identities before approval. Delete revalidates a direct single-link source; move is no-clobber and same-filesystem with explicit `PARTIAL_PUBLICATION` and no rollback that could delete a raced path.
- `hashline_write` is strict and accepts only `filePath` and `content`; the obsolete `createParents` field is rejected. Pin the deepest existing ancestor for path identity, then freeze and authorize a complete reservation containing at most 64 missing directories plus the target, retaining locks for those reserved mutation names. Before edit permission, adopt only an exact reverified contiguous root-side prefix that appeared while waiting; report verified reuse as success, create only the remaining suffix with exclusive non-recursive `mkdir`, then use pinned staged no-clobber publication. Never add/substitute paths. After this call creates a directory or a `mkdir` outcome becomes ambiguous, fail as `PARTIAL_PUBLICATION` without rollback. `move_file` never creates parents.
- Use small delayed abort-aware retries only for complete read-only observations and exact owned-staging cleanup/handle closure. Never retry target publication syscalls. Persistent drift, provenance loss, and publication ambiguity remain model-visible errors.
- Partial move or parent publication advances per-path generation fences so already queued overlapping calls stop before mutation, invalidates affected snapshots, and unbinds the native-alias live epoch. After inspecting and repairing paths, a fresh delivered `hashline_read` may rebind in the same session; old snapshot IDs never revive.
...
- The current schema-v10 model-free result preserves the unchanged schema-v9 29-case classifications and adds cumulative-coverage-header and explicit `replace_file`-readback wire fixtures; strict-only defaults remain runtime-test evidence, not corpus evidence. Keep schema-v5 through schema-v9 and pilot-v7 evidence immutable, retain schema-v10 only at its dated write-once path, never overwrite retained evidence, and make no semantic-code, paid, or model-quality claim from deterministic output.
