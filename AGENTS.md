...
- No Better Hashline failure or resource limit requires abandoning the OpenCode transcript or task ID. Recover in the same task by retrying, taking a fresh delivered read, repairing paths/configuration, restarting the plugin or host as applicable and rereading, or explicitly switching configuration to enforced `toolSurface:"hashline"`; this is never a silent fallback. This invariant is scoped to Better Hashline and does not cover loss of OpenCode's own session database.
