...
export const PILOT_V7_ID = "native-alias-pilot-v7" as const;
export const PILOT_V7_APPROVAL_ANCHOR_PATH =
  "benchmarks/model/native-alias-pilot-v7.approval.json" as const;
export const PILOT_V7_OUTPUT_RELATIVE_PATH =
  "benchmarks/results/model/native-alias-pilot-v7" as const;
export const PILOT_V7_PREFLIGHT_SCHEMA_VERSION = 6 as const;
export const PILOT_V7_TASK_MANIFEST_SHA256 =
  "5465f2c98800241ec031375ee11d72f30b8649c00c8196359ba1b6dd39cef3ca" as const;
export const PILOT_V7_ADAPTER_MANIFEST_SHA256 =
  "cdd7ed43f920aeb7d883445095cdf2930372fc76ab9e52ec3ac122784eb8ccb8" as const;
export const PILOT_V7_SCHEDULE_MANIFEST_SHA256 =
  "3b694becb988e6fcd1dace046ad45e298cdc4f4600d512ab54e3bb8a3cfdb70d" as const;
export const PILOT_V7_PACKAGE_VERSION = "0.2.1" as const;

export const PILOT_V7_LIMITS = {
  repeats: 1,
  maxAgentSteps: 12,
...
export const PILOT_V7_RESERVATION_PROTOCOL =
  "opencode-better-hashline-paid-pilot-reservation/v1" as const;
export const PILOT_V7_RESERVATION_NAMESPACE =
  "io.github.makcimbx.opencode-better-hashline" as const;
export const PILOT_V7_RESERVATION_KEY = PILOT_V7_ID;
export const PILOT_V7_RESERVATION_ID =
  `${PILOT_V7_RESERVATION_NAMESPACE}/${PILOT_V7_RESERVATION_KEY}` as const;

export const PILOT_V7_BROKER_TIMEOUT_MS = 10_000 as const;
export const PILOT_V7_BROKER_STDOUT_LIMIT = 64 * 1024;
export const PILOT_V7_BROKER_STDERR_LIMIT = 16 * 1024;
...
export interface ActivePilotV7ApprovalAnchor {
  schemaVersion: typeof APPROVAL_SCHEMA_VERSION;
  pilotId: typeof PILOT_V7_ID;
  approval: {
...
export type PilotV7ApprovalAnchor =
  | {
      schemaVersion: typeof APPROVAL_SCHEMA_VERSION;
      pilotId: typeof PILOT_V7_ID;
      approval: null;
    }
  | ActivePilotV7ApprovalAnchor;

export interface PilotV7ExternalApprovalBundle {
  schemaVersion: typeof EXTERNAL_BUNDLE_SCHEMA_VERSION;
  pilotId: typeof PILOT_V7_ID;
  candidateCommit: string;
  packageVersion: typeof PILOT_V7_PACKAGE_VERSION;
  preflightSchemaVersion: typeof PILOT_V7_PREFLIGHT_SCHEMA_VERSION;
  hashes: {
  ...
  outputRelativePath: typeof PILOT_V7_OUTPUT_RELATIVE_PATH;
  limits: typeof PILOT_V7_LIMITS;
  reservation: {
    protocol: typeof PILOT_V7_RESERVATION_PROTOCOL;
    namespace: typeof PILOT_V7_RESERVATION_NAMESPACE;
    key: typeof PILOT_V7_RESERVATION_KEY;
    authority: string;
  };
}

export interface ValidatedPilotV7ApprovalCommit {
  approvalCommit: string;
  candidateCommit: string;
  anchor: ActivePilotV7ApprovalAnchor;
}

export interface ValidatedPilotV7Approval extends ValidatedPilotV7ApprovalCommit {
  externalBundleSha256: string;
  bundle: PilotV7ExternalApprovalBundle;
}

export interface PilotV7ReservationRequest {
  schemaVersion: typeof RESERVATION_REQUEST_SCHEMA_VERSION;
  operation: "consume";
  protocol: typeof PILOT_V7_RESERVATION_PROTOCOL;
  namespace: typeof PILOT_V7_RESERVATION_NAMESPACE;
  key: typeof PILOT_V7_RESERVATION_KEY;
  reservationId: typeof PILOT_V7_RESERVATION_ID;
  authority: string;
  pilotId: typeof PILOT_V7_ID;
  candidateCommit: string;
  approvalCommit: string;
...
export interface PilotV7ReservationReceipt {
...
export interface BrokerInvocation {
  ...
  cwd: string;
  timeoutMs: typeof PILOT_V7_BROKER_TIMEOUT_MS;
  stdoutLimit: number;
  stderrLimit: number;
}
...
function committedAnchorEncoding(value: unknown): string {
  ...
  if (
    ...
    root.pilotId !== PILOT_V7_ID
  ) {
    return `${canonicalJson(value)}\n`;
  }
  if (root.approval === null) {
    return `{ "approval": null, "pilotId": "${PILOT_V7_ID}", "schemaVersion": 1 }\n`;
  }
...
export function parsePilotV7ApprovalAnchor(bytes: Uint8Array): PilotV7ApprovalAnchor {
  const value = record(
    parseCanonicalJson(
      bytes,
      "Pilot v7 approval anchor",
      ANCHOR_BYTE_LIMIT,
  ...
  if (
    ...
    value.pilotId !== PILOT_V7_ID
  ) {
    throw new Error("Pilot v7 approval anchor has an invalid exact-key schema.");
  }
  if (value.approval === null) return value as unknown as PilotV7ApprovalAnchor;
  ...
  if (
    ...
    throw new Error("Pilot v7 active approval anchor has an invalid exact-key schema.");
  }
  return value as unknown as ActivePilotV7ApprovalAnchor;
}

export function parsePilotV7ExternalApprovalBundle(
  bytes: Uint8Array,
): PilotV7ExternalApprovalBundle {
  const value = record(
    parseCanonicalJson(bytes, "Pilot v7 external approval bundle", BUNDLE_BYTE_LIMIT),
  );
  ...
  if (
    ...
    value.packageVersion !== PILOT_V7_PACKAGE_VERSION ||
    value.preflightSchemaVersion !== PILOT_V7_PREFLIGHT_SCHEMA_VERSION ||
    value.outputRelativePath !== PILOT_V7_OUTPUT_RELATIVE_PATH ||
    !hashes ||
    !exactKeys(hashes, HASH_KEYS) ||
    !HASH_KEYS.every((key) => isSha256(hashes[key])) ||
    hashes.taskManifestSha256 !== PILOT_V7_TASK_MANIFEST_SHA256 ||
    hashes.adapterManifestSha256 !== PILOT_V7_ADAPTER_MANIFEST_SHA256 ||
    hashes.scheduleManifestSha256 !== PILOT_V7_SCHEDULE_MANIFEST_SHA256 ||
    !limits ||
    !exactKeys(limits, LIMIT_KEYS) ||
    !LIMIT_KEYS.every((key) => limits[key] === PILOT_V7_LIMITS[key]) ||
    !reservation ||
    !exactKeys(reservation, RESERVATION_KEYS) ||
    reservation.protocol !== PILOT_V7_RESERVATION_PROTOCOL ||
    reservation.namespace !== PILOT_V7_RESERVATION_NAMESPACE ||
    reservation.key !== PILOT_V7_RESERVATION_KEY ||
    typeof reservation.authority !== "string" ||
    !/^[\x21-\x7e]{1,256}$/u.test(reservation.authority)
  ) {
    throw new Error(
      "Pilot v7 external approval bundle does not match the frozen exact-key schema.",
    );
  }
  return value as unknown as PilotV7ExternalApprovalBundle;
}
...
export async function loadCommittedPilotV7ApprovalAnchor(
  input: { repository: string; commit: string },
  dependencies: ApprovalDependencies = {},
): Promise<PilotV7ApprovalAnchor> {
  ...
  const bytes = await gitBytes(
    input.repository,
    ["show", `${input.commit}:${PILOT_V7_APPROVAL_ANCHOR_PATH}`],
    runGit,
  );
  return parsePilotV7ApprovalAnchor(bytes);
}

export async function validatePilotV7ApprovalCommit(
  input: { repository: string; approvalCommit: string },
  dependencies: ApprovalDependencies = {},
): Promise<ValidatedPilotV7ApprovalCommit> {
  if (!isCommit(input.approvalCommit)) {
    throw new Error("Pilot v7 approval commit must be exact 40-hex.");
  }
  const runGit = dependencies.runGit ?? defaultGitRunner;
  const head = (
  ...
  if (head !== input.approvalCommit) {
    throw new Error("Pilot v7 approval commit must be the checked-out HEAD.");
  }
  ...
  if (status.length !== 0) {
    throw new Error("Pilot v7 approval commit requires a clean worktree.");
  }
  ...
  if (hiddenIndexFlags.length !== 0) {
    throw new Error("Pilot v7 approval commit forbids hidden index flags.");
  }
  ...
  if (lineage.length !== 2 || lineage[0] !== input.approvalCommit || !isCommit(lineage[1])) {
    throw new Error("Pilot v7 approval commit must have exactly one parent candidate A.");
  }
  ...
  if (changedPaths.length !== 1 || changedPaths[0] !== `M\t${PILOT_V7_APPROVAL_ANCHOR_PATH}`) {
    throw new Error("Candidate A..approval C must change only the pilot v7 approval anchor.");
  }

  const anchor = await loadCommittedPilotV7ApprovalAnchor(
  ...
  if (anchor.approval.candidateCommit !== candidateCommit) {
    throw new Error("Pilot v7 approval anchor candidate does not match parent A.");
  }
  const candidateAnchor = await loadCommittedPilotV7ApprovalAnchor(
    { repository: input.repository, commit: candidateCommit },
    { runGit },
  );
  if (candidateAnchor.approval !== null) {
    throw new Error("Candidate A must contain the hard-disabled null pilot v7 approval anchor.");
  }
  return { approvalCommit: input.approvalCommit, candidateCommit, anchor };
}

export function validatePilotV7ExternalApprovalBundle(
  bytes: Uint8Array,
  approval: ValidatedPilotV7ApprovalCommit,
): ValidatedPilotV7Approval {
  const externalBundleSha256 = sha256(bytes);
  if (externalBundleSha256 !== approval.anchor.approval.externalBundleSha256) {
    throw new Error("Pilot v7 external approval bundle hash does not match the committed anchor.");
  }
  const bundle = parsePilotV7ExternalApprovalBundle(bytes);
  if (bundle.candidateCommit !== approval.candidateCommit) {
    throw new Error("Pilot v7 external approval bundle candidate does not match parent A.");
  }
...
function reservationRequest(approval: ValidatedPilotV7Approval): PilotV7ReservationRequest {
  return {
    schemaVersion: RESERVATION_REQUEST_SCHEMA_VERSION,
    operation: "consume",
    protocol: PILOT_V7_RESERVATION_PROTOCOL,
    namespace: PILOT_V7_RESERVATION_NAMESPACE,
    key: PILOT_V7_RESERVATION_KEY,
    reservationId: PILOT_V7_RESERVATION_ID,
...
function parseReservationReceipt(
...
): PilotV7ReservationReceipt {
  const value = record(
    parseCanonicalJson(bytes, "External reservation broker response", PILOT_V7_BROKER_STDOUT_LIMIT),
  );
  if (
    !value ||
    ...
    value.protocol !== PILOT_V7_RESERVATION_PROTOCOL ||
    value.namespace !== PILOT_V7_RESERVATION_NAMESPACE ||
    value.key !== PILOT_V7_RESERVATION_KEY ||
    value.reservationId !== PILOT_V7_RESERVATION_ID ||
  ...
  return value as unknown as PilotV7ReservationReceipt;
}
...
): Promise<PilotV7ReservationReceipt> {
  const approvalCommit = await validatePilotV7ApprovalCommit(
    { repository: input.repository, approvalCommit: input.approvalCommit },
    dependencies,
  );
  const approval = validatePilotV7ExternalApprovalBundle(input.externalBundleBytes, approvalCommit);
...
export async function consumeValidatedExternalReservation(
  input: {
    repository: string;
    approval: ValidatedPilotV7Approval;
    brokerPath: string;
    repositoryAndWorktreeRoots: readonly string[];
  },
  ...
  try {
    ...
    result = await invokeBroker({
      ...
      cwd: privateRoot,
      timeoutMs: PILOT_V7_BROKER_TIMEOUT_MS,
      stdoutLimit: PILOT_V7_BROKER_STDOUT_LIMIT,
      stderrLimit: PILOT_V7_BROKER_STDERR_LIMIT,
    });
  } finally {
