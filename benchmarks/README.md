...
The runner emits schema v9. The latest retained schema-v9 result keeps the 29-case corpus and adds an
omitted/default adapter that uses the incremental branch of the shared runtime policy resolver. Its retained
classifications are strict `6/18/5/0`, explicit unique `11/18/0/0`, omitted/default `11/18/0/0`,
exact search `10/13/1/5`, line numbers `7/1/0/21`, endpoint-8 `7/12/4/6`, and endpoint-16
`7/13/4/5` (`exact_apply/safe_reject/false_reject/unsafe_accept`). Its retained fixture wire values
are edit schema `5035 -> 6637` (+1602, 31.82%), write schema `849 -> 531` (-318, -37.46%), inferred
...
claims. Strict-only defaults are covered by runtime tests rather than this corpus.

The latest retained model-free evidence is the immutable schema-v9
...
The schema-v5, schema-v6, schema-v7, and schema-v8 records remain immutable.
