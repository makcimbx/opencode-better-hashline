...
import {
  ...
  type GitRunner,
  PILOT_V7_ADAPTER_MANIFEST_SHA256,
  PILOT_V7_APPROVAL_ANCHOR_PATH,
  PILOT_V7_ID,
  PILOT_V7_LIMITS,
  PILOT_V7_OUTPUT_RELATIVE_PATH,
  PILOT_V7_PACKAGE_VERSION,
  PILOT_V7_PREFLIGHT_SCHEMA_VERSION,
  PILOT_V7_RESERVATION_ID,
  PILOT_V7_RESERVATION_KEY,
  PILOT_V7_RESERVATION_NAMESPACE,
  PILOT_V7_RESERVATION_PROTOCOL,
  PILOT_V7_SCHEDULE_MANIFEST_SHA256,
  PILOT_V7_TASK_MANIFEST_SHA256,
  type PilotV7ApprovalAnchor,
  type PilotV7ExternalApprovalBundle,
  parsePilotV7ApprovalAnchor,
  parsePilotV7ExternalApprovalBundle,
...
function nullAnchor(): PilotV7ApprovalAnchor {
  return { schemaVersion: 1, pilotId: PILOT_V7_ID, approval: null };
}

function activeAnchor(
  candidateCommit: string,
  externalBundleBytes: Uint8Array,
): PilotV7ApprovalAnchor {
  return {
    schemaVersion: 1,
    pilotId: PILOT_V7_ID,
    approval: {
...
function anchorBytes(anchor: PilotV7ApprovalAnchor): Buffer {
  if (anchor.approval === null) {
    return Buffer.from(
      `{ "approval": null, "pilotId": "${PILOT_V7_ID}", "schemaVersion": 1 }\n`,
      "utf8",
    );
  }
  return Buffer.from(`{
...
function externalBundle(brokerExecutableSha256: string): PilotV7ExternalApprovalBundle {
  return {
    schemaVersion: 1,
    pilotId: PILOT_V7_ID,
    candidateCommit: CANDIDATE,
    packageVersion: PILOT_V7_PACKAGE_VERSION,
    preflightSchemaVersion: PILOT_V7_PREFLIGHT_SCHEMA_VERSION,
    hashes: {
      ...
      scheduleManifestSha256: PILOT_V7_SCHEDULE_MANIFEST_SHA256,
      taskManifestSha256: PILOT_V7_TASK_MANIFEST_SHA256,
      adapterManifestSha256: PILOT_V7_ADAPTER_MANIFEST_SHA256,
    ...
    outputRelativePath: PILOT_V7_OUTPUT_RELATIVE_PATH,
    limits: { ...PILOT_V7_LIMITS },
    reservation: {
...
interface FakeGitOptions {
  currentAnchor: PilotV7ApprovalAnchor;
  parentAnchor?: PilotV7ApprovalAnchor;
  head?: string;
  status?: string;
...
function fakeGit(options: FakeGitOptions): GitRunner {
  return ({ args }) => {
    ...
    if (args[0] === "diff") {
      return `${(options.changedPaths ?? [`M\t${PILOT_V7_APPROVAL_ANCHOR_PATH}`]).join("\n")}\n`;
    }
    if (args[0] === "show") {
      if (args[1] === `${APPROVAL_COMMIT}:${PILOT_V7_APPROVAL_ANCHOR_PATH}`) {
        return anchorBytes(options.currentAnchor);
      }
      if (args[1] === `${CANDIDATE}:${PILOT_V7_APPROVAL_ANCHOR_PATH}`) {
...
describe("pilot v7 external approval", () => {
  test("freezes the exact v7 identity, evidence hashes, and execution limits", () => {
    expect(PILOT_V7_ID).toBe("native-alias-pilot-v7");
    expect(PILOT_V7_APPROVAL_ANCHOR_PATH).toBe(
      "benchmarks/model/native-alias-pilot-v7.approval.json",
    );
    expect(PILOT_V7_OUTPUT_RELATIVE_PATH).toBe("benchmarks/results/model/native-alias-pilot-v7");
    expect(PILOT_V7_PACKAGE_VERSION).toBe("0.2.1");
    expect(PILOT_V7_PREFLIGHT_SCHEMA_VERSION).toBe(6);
    expect(PILOT_V7_TASK_MANIFEST_SHA256).toBe(
      "5465f2c98800241ec031375ee11d72f30b8649c00c8196359ba1b6dd39cef3ca",
    );
    expect(PILOT_V7_ADAPTER_MANIFEST_SHA256).toBe(
      "cdd7ed43f920aeb7d883445095cdf2930372fc76ab9e52ec3ac122784eb8ccb8",
    );
    expect(PILOT_V7_SCHEDULE_MANIFEST_SHA256).toBe(
      "3b694becb988e6fcd1dace046ad45e298cdc4f4600d512ab54e3bb8a3cfdb70d",
    );
    expect(PILOT_V7_LIMITS).toEqual({
      repeats: 1,
      maxAgentSteps: 12,
    ...
    expect(PILOT_V7_RESERVATION_NAMESPACE).toBe("io.github.makcimbx.opencode-better-hashline");
    expect(PILOT_V7_RESERVATION_KEY).toBe("native-alias-pilot-v7");
    expect(PILOT_V7_RESERVATION_ID).toBe(
      "io.github.makcimbx.opencode-better-hashline/native-alias-pilot-v7",
    );
  });

  test("ships the exact canonical null approval anchor", async () => {
    const anchor = parsePilotV7ApprovalAnchor(await readFile(PILOT_V7_APPROVAL_ANCHOR_PATH));
  ...
  test("requires clean single-parent C and an A-to-C anchor-only activation", async () => {
    ...
    for (const runGit of [
      ...
      fakeGit({
        currentAnchor,
        changedPaths: [`M\t${PILOT_V7_APPROVAL_ANCHOR_PATH}`, "M\tbenchmarks/model/run.ts"],
      }),
      fakeGit({
